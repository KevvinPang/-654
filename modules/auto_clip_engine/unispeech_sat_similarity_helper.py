...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        segment_embedding_cache: Dict[Tuple[float, float, float, float, bool], object] = {}

        def encode_cached(item: Dict[str, object]):
            start = float(item.get("start", 0.0) or 0.0)
            end = float(item.get("end", 0.0) or 0.0)
            cache_key = (
                round(start, 4),
                round(end, 4),
                round(float(item.get("pad_seconds", 0.08) or 0.0), 4),
                round(float(item.get("min_duration", 0.45) or 0.45), 4),
                bool(item.get("allow_context_extension", True)),
            )
        ...
        encoded_entries: List[Tuple[Dict[str, object], int, object]] = []
        for item in entries:
            try:
            ...
            try:
                ...
                continue
            encoded_entries.append((item, index, vector))

        output_entries: List[Dict[str, object]] = []
        for position, (_item, index, vector) in enumerate(encoded_entries):
            ...
            if position > 0:
                previous_index = encoded_entries[position - 1][1]
                previous_vector = encoded_entries[position - 1][2]
                output_item["previous_index"] = previous_index
                output_item["previous_similarity"] = round(_cosine_similarity(vector, previous_vector), 6)
            if position + 1 < len(encoded_entries):
                next_index = encoded_entries[position + 1][1]
                next_vector = encoded_entries[position + 1][2]
                output_item["next_index"] = next_index
                output_item["next_similarity"] = round(_cosine_similarity(vector, next_vector), 6)
            output_entries.append(output_item)

        output_by_index = {int(item["index"]): item for item in output_entries}
        for item in output_entries:
            previous = output_by_index.get(int(item.get("previous_index", 0) or 0))
            next_item = output_by_index.get(int(item.get("next_index", 0) or 0))
            if previous is not None:
                item["previous_narration_similarity"] = previous.get("narration_similarity", 0.0)
                item["previous_dialogue_similarity"] = previous.get("dialogue_similarity", 0.0)
            if next_item is not None:
                item["next_narration_similarity"] = next_item.get("narration_similarity", 0.0)
                item["next_dialogue_similarity"] = next_item.get("dialogue_similarity", 0.0)
