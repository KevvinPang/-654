...
$moduleDir = Join-Path $root 'modules\auto_clip_engine'
...
if (Test-Path -LiteralPath (Join-Path $venvDir 'Scripts\python.exe')) {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to read the existing auto clip virtual environment version."
    }
...
if (-not (Test-Path -LiteralPath $venvDir)) {
    Invoke-Checked -FilePath $selectedPython -Arguments @('-m', 'venv', $venvDir) -ErrorMessage "Failed to create the auto clip virtual environment."
}
...
Invoke-Checked -FilePath $venvPython -Arguments @('-m', 'pip', 'install', '--upgrade', 'pip') -ErrorMessage "Failed to upgrade pip for the auto clip environment."
Invoke-Checked -FilePath $venvPython -Arguments @('-m', 'pip', 'install', '-r', $requirements) -ErrorMessage "Failed to install auto clip dependencies."

$pythonVersion = & $venvPython -c "import sys; print(f'{sys.version_info.major}.{sys.version_info.minor}')"
if ($LASTEXITCODE -ne 0) {
    throw "Failed to confirm the auto clip environment Python version."
}

Write-Host "Auto clip environment is ready."
